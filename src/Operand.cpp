...
template<class T> const IOperand* Operand<T>::operator / (const IOperand& rhs) const
{
	...
	if (this->getPrecision() < rhs.getPrecision())
	{
		switch (rhs.getPrecision())
		{
			case e_int16 :	if (this->_val == std::numeric_limits<int16_t>::min() && rhs_val == -1)
								throw std::overflow_error("Division Overflow");
							return new Operand<int16_t>(static_cast<int16_t>(this->_val) / rhs_val, rhs.getType());
			case e_int32 :	if (this->_val == std::numeric_limits<int32_t>::min() && rhs_val == -1)
								throw std::overflow_error("Division Overflow");
							return new Operand<int32_t>(static_cast<int32_t>(this->_val) / rhs_val, rhs.getType());
	...
	if (this->getPrecision() <= e_int32 && this->_val == std::numeric_limits<T>::min() && rhs_val == -1)
		throw std::overflow_error("Division Overflow");
...
template<class T> const IOperand* Operand<T>::operator % (const IOperand& rhs) const
{
	...
	if (this->getType() > e_int32 || rhs.getType() > e_int32)
		throw std::invalid_argument("Invalid Modulus Type");
	if (this->_val == std::numeric_limits<T>::min() && rhs_val == -1)
		throw std::overflow_error("Modulus Overflow");

	switch (this->getPrecision() < rhs.getPrecision() ? rhs.getPrecision() : this->getPrecision())
	{
		case e_int8  : return new Operand<int8_t>(static_cast<int8_t>(this->_val) % rhs_val, rhs.getType());
	...
	return new Operand<int32_t>(static_cast<int32_t>(this->_val) % rhs_val, this->getType());
}
